...
#Más específico toda la función " def dibujar(): "
...
def dibujarCuadrados_y_Circulos(ventana): # Dibuja cuadrados y circulos con una separación de 10 pixeles
...
def dibujarParabolas(ventana): #Dibuja un tipo de rombo con parábolas con colores aleatorios
...
def dibujarEspiral(ventana): #Dibuja un espiral empezando de la esquina inferior derecha
...
def dibujarCirculos(ventana): #Dibuja 12 círculos desde el perímetro de un círculo con el mismo radio
...
def dibujar(eleccion): #Todo lo necesario para que en la ventana se pueda dibujar.
    ...
    while not termina:  # Ciclo principal, MIENTRAS la variabel termina sea False, el ciclo se repite automáticamente
        ...
        if eleccion == 1:
            dibujarCuadrados_y_Circulos(ventana)
        elif eleccion == 2:
